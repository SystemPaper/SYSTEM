package org.tool.system.inputConfig.phase.linking.similarity_measuring;
...
public class SimilarityComponent {
    ...
    public SimilarityComponent(Element PhaseContent){
        ...
        switch (method){
            ...
            case LONGEST_COMMON_SUBSTRING:
                ...
                secondSimilarityComputationMethod = SimilarityComputationMethod.valueOf(PhaseContent.getElementsByTagName("secondSimilarityComputationMethod").item(0).getTextContent());
                break;
            case MONGE_ELKAN:
                stringParam = PhaseContent.getElementsByTagName("StringParam").item(0).getTextContent();
                doubleParam = Double.parseDouble(PhaseContent.getElementsByTagName("DoubleParam").item(0).getTextContent());
    ...
    public org.tool.system.linking.similarity_measuring.data_structures.SimilarityComponent toSimilarityComponent (){
        switch (method){
            case EXTENDED_JACCARD:
...
//            case NUMERICAL_SIMILARITY_MAXDISTANCE:
//                return new NumericalSimilarityWithMaxDistanceComponent(id, method, srcGraphLabel, srcLabel, srcAttribute, targetGraphLabel, targetLabel, targetAttribute, weight, doubleParam);
//            case NUMERICAL_SIMILARITY_MAXPERCENTAGE:
//                return new NumericalSimilarityWithMaxPercentageComponent(id, method, srcGraphLabel, srcLabel, srcAttribute, targetGraphLabel, targetLabel, targetAttribute, weight, doubleParam);
            case QGRAMS:
...
//            case EDIT_DISTANCE:
//                return new SimilarityComponent(id, method, srcGraphLabel, srcLabel, srcAttribute, targetGraphLabel, targetLabel, targetAttribute, weight);
        }
        return null;
    }
}
