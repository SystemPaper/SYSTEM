...
public class ComputeSimGraphQualityMeasuresWithoutPM {
    ...
    public ComputeSimGraphQualityMeasuresWithoutPM(DataSet<Vertex> vertices,  DataSet<Edge> edges, String VertexIdLabel) {
...
//                "*******************************************************************************************************edges: " + inputGrph.getEdges().count());
        ...
        DataSet<Tuple2<GradoopId, String>> vertexIdPubId = vertices.map(new MapFunction<Vertex, Tuple2<GradoopId, String>>() {
            public Tuple2<GradoopId, String> map(Vertex in) {
//                String recId = in.getPropertyValue("clusterId").toString();
                String recId = in.getPropertyValue("clsId").toString().split("s")[0];
        ...
        DataSet<Tuple2<GradoopId, GradoopId>> sourceIdtargetId = edges.map(new MapFunction<Edge, Tuple2<GradoopId, GradoopId>>() {
        ...
        apset = Count.count(edges).map(new MapFunction<Long, Tuple2<String, Long>>() {
