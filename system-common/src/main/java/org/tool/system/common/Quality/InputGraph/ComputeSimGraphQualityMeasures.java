...
public class ComputeSimGraphQualityMeasures {
    ...
    private DataSet<Vertex> inputVertices;
    private DataSet<Edge> inputEdges;
    ...
    public ComputeSimGraphQualityMeasures(String groundTruthFilepath, String groundTruthSplitter, LogicalGraph inputGraph, String VertexIdLabel) {
        ...
        inputVertices = inputGraph.getVertices();
        inputEdges = inputGraph.getEdges();
    ...
    public ComputeSimGraphQualityMeasures(String groundTruthFilepath, String groundTruthSplitter, GraphCollection inputCol, String VertexIdLabel) {
        GTPath = groundTruthFilepath.trim();
        GTSplitter = groundTruthSplitter;
        inputVertices = inputCol.getVertices();
        inputEdges = inputCol.getEdges();        vertexIdLabel = VertexIdLabel;
        tpset = apset = gtRecorsNoSet = null;
        ap = tp = gtRecorsNo = -1L;
        env = inputCol.getConfig().getExecutionEnvironment();
...
//        });
        ...
        DataSet<Tuple2<GradoopId, String>> vertexIdPubId = inputVertices.map(new GetVertexIdPubId(vertexIdLabel));
        DataSet<Tuple2<GradoopId, String>> vetexIdClusterId = DataSetUtils.zipWithUniqueId(inputEdges.map(new MapFunction<Edge, Tuple2<GradoopId, GradoopId>>() {
...
//        System.out.println("with.count: "+with.count());
        ...
        */
        ...
        apset = Count.count(inputEdges).map(new MapFunction<Long, Tuple2<String, Long>>() {
            public Tuple2<String, Long> map(Long a) {
...
//            System.out.println("tammam "+i.f0);
    ...
    private class GetVertexIdPubId implements Serializable, MapFunction<Vertex, Tuple2<GradoopId, String>> {
        String vertexIdLabel;
        public GetVertexIdPubId(String vertexIdLabel) {
            this.vertexIdLabel = vertexIdLabel;
        }
        public Tuple2<GradoopId, String> map(Vertex in) {
                return Tuple2.of(in.getId(), in.getPropertyValue(this.vertexIdLabel).toString());
        }
